...
# signal handler (trigger for the action - buy or sell)

# inputs for creating a trade signal

# trade pattern - breakout or break down. These can include triangles, rectangles, head & shoulders, and trendlines
# moving average cross - a few different things can happen here. The input could be when trading activity crosses above or below a certain level (average)
# volume surge - unusually high volume is often a precursor to view a new move in the market. (i.e. futures market: open interest)
# interest rates - changes in rates can often suggest changes in different areas (stock and commodity markets)
# volatility - there are many ways to measure volatility and as with other indicators. (extreme highs or lows in volatility) - implying market change
# cycles - markets of all types tend to ebb and flow over time, even if they are in steady trend or in a non trending condition
# setiment - (used as a contrarian indicator), excessive bullishness according to surveys or actual trading activity can suggest market tops
# valuation - an excessively high valuation compared to market, sector, or stock specific measure can lead to sell signals
...
#  return the bid and ask price (at the exact amount) - what is this
...
# Trading signal:
# ticker_bid price > 1.03 * ticker_ask theoretical price: sell this ticker
...
# Return volatility from the latest current events or news:
